...
\end{problem}
\begin{problem}
  У Васи есть два одинаковых золотых слитка неизвестной массы $m$ каждый и весы, которые взвешивают с некоторой погрешностью. Сначала Вася положил на весы один слиток и получил результат $Y_1=m+u_1$, где $u_1$ — случайная величина, ошибка первого взвешивания. Затем Вася положил на весы сразу оба слитка и получил результат $Y_2=2m+u_2$, где $u_2$ — случайная величина, ошибка второго взвешивания. Оказалось, что $y_1=0.9$, а $y_2=2.3$.
...
\begin{problem}
  Допустим, что $X_i$ — независимы и имеют закон распределения, заданный табличкой: \\
...
$[0.1;0.4]$. Как выглядит условное распределение $\theta$, если известно что $X_1 = 0$, $X_2 = 2$? \\
\item Постройте ML и MM оценки для произвольной выборки $X_1$, $X_2$, \ldots $X_n$
...
\end{problem}
\begin{problem}
  Пусть $Y_1$ и $Y_2$ независимы и распределены по Пуассону. Известно также, что $\E(Y_1)=e^{a}$ и $\E(Y_2)=e^{a+b}$.
\begin{enumerate}
...
\end{problem}
\begin{problem}
  Пусть  $X_1$, \ldots , $X_n$  распределены одинаково и независимо.
Оцените значение $\theta $ с помощью ML (везде) и MM (в «а» и «б»), оцените дисперсию ML оценки, если функция плотности $X_i$, $p(t)$ имеет вид:
\begin{enumerate}
...
\end{problem}
\begin{problem}
  Пусть $X_1$, \ldots , $X_n$ - независимы и экспоненциальны с параметром $\lambda$. Постройте MM и ML оценки параметра  $\lambda$. Оцените дисперсию ML оценки.

\begin{sol}
\end{sol}
\end{problem}
\begin{problem}
  Пусть $X_1$, \ldots , $X_n$ - независимы и $\cN(\mu; \sigma^2)$. 
  Значение $\sigma^2$ известно.  
  Постройте MM и ML оценки параметра $\mu$.
...
\begin{problem}
   Пусть $X_1$, $X_2$,\ldots , $X_n$ независимы и
равномерно
...
знает значений самих $X_i$, а знает только количество $X_i$
...
\begin{problem}
  Известно, что $X_i$ — независимы и имеют закон распределения, заданный таблицей: \\
\begin{tabular}{*{3}{c}}
\toprule
$X_i$ & 0 & 1 \\
\midrule
...
\begin{problem}
  Пусть $X_i$ независимы и имеют экспоненциальное распределение с параметром
$\lambda$, т.е. $p(t)=\lambda e^{-\lambda t}$.
\begin{enumerate}
\item Найдите $I(\lambda)$, если наблюдаются $X_1$, \ldots , $X_n$ \\
\item Пусть $\lambda=1/\theta$, т.е. $p(t)=\frac{1}{\theta}
e^{-\frac{1}{\theta} t}$. Найдите $I(\theta)$, если наблюдается $X_1$, \ldots , $X_n$
...
\begin{problem}
  Пусть $X_i$ — независимы и одинаково распределены. Пусть $I_{X_i}(\theta)$ - информация Фишера о $\theta$, получаемая при наблюдении $X_i$.
\begin{enumerate}
...
\left[1;e\right]$. По выборке из 100 наблюдений оказалось, что $\sum{\ln(\ln(X_i))}=-30$
\begin{enumerate}
...
Подсказка: $\E(1/X_i)=1/\theta^2$ (интеграл берется заменой $x=\theta^2a^{-2}$)
\end{enumerate}
