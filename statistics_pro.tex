...
содержащей $N_{-1}$ значений $X_i = -1$, $N_0$ значений $X_i = 0$ и $N_2$ значений $X_i = 2$.
...
\item Решите пункты (а), (б) и (в) для плотности $\frac{2t}{\theta ^2}$  при  $t\in [0;\theta]$.
...
\left[1;e\right]$.
\item Решите пункты (а) и (б) для плотности $f(x) = \frac{e^{-\abs{x}} }{2(1-e^{-\theta })} $
при  $x\in [-\theta ;\theta ]$.
\end{enumerate}
...
\begin{problem}
  ...
  \[
  f(x) = \begin{cases}
    \frac{\theta e^{-\frac{\theta^2}{2x}}}{\sqrt{2\pi x^{3}}} \text{ при } x\in [0;+\infty), \\
    0, \text{ иначе.}
  \end{cases}
  \]
