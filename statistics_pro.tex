...
\begin{leftbar}
  ...
  Если $Z$ имеет нормальное распределение $\cN(0;1)$ и $Q$ имеет хи-квадрат распределение $\chi_2^d$, $Z$ и $Q$ независимы,
  \[
      R = \frac{Z}{\sqrt{Q / k}},
  \]
...
\begin{problem}
 Кот Матроскин каждый вечер ходит на рыбалку. Поймав одну «рыбку» кот Матроскин возвращается домой. 
 В пруду встречаются караси, щуки и бегемоты. Кот Матроскин хочет оценить вероятность $p$ поймать карася. 
...
Величина $Y_i$ равна 1 если при втором отлове $i$-ый заяц оказался с бантиком и 0 иначе.
...
%Если $X_i$ - независимы и одинаково распределены, то $I(\theta)=n %I_{X_i}(\theta)$, где $I_{X_i}(\theta)$ - информация о неизвестном $\theta$, %содержащаяся в наблюдении одного $X_i$ \\
...
\toprule
 x & -1 & 0 & 2 \\
\midrule
 $\P(X = x)$ & $\theta$ & $2\theta-0.2$ & $1.2-3\theta$ \\
\bottomrule
\end{tabular}
...
что $\theta$ распределена равномерно на отрезке $[0.1;0.4]$. 
...
\item Решите пункты (а), (б) и (в) для плотности $\frac{2t}{\theta ^2} $  при  $t\in \left[0;\theta \right]$.
\item Решите пункты (а) и (б) для плотности $\frac{\theta e^{-\frac{\theta ^2 }{2t} } }{\sqrt{2\pi t^{3}
} } $  при  $t\in [0;+\infty)$.
\item Решите пункты (а) и (б) для плотности $\frac{\theta \left(\ln ^{\theta -1} t\right)}{t} $  при  $t\in
\left[1;e\right]$.
\item Решите пункты (а) и (б) для плотности $\frac{e^{-\left|t\right|} }{2\left(1-e^{-\theta } \right)} $
...
\begin{enumerate}
  \item Постройте оценку $\hat\alpha_{ML}$ методом максимального правдоподобия. 
  \item Оцените дисперсию оценки $\hat \alpha_{ML}$ максимального правдоподобия.
  \item Постройте оценку $\hat\alpha_{MM}$ методом моментов. 
  \item Оцените дисперсию оценки $\hat \alpha_{MM}$ метода моментов.
\end{enumerate}
...
\begin{problem}
  У нас есть всего одно наблюдение $Y_1\sim N(0;\frac{1}{1-\theta^2})$. 
  \begin{enumerate}
...
\]
\begin{enumerate}
  \item Постройте оценку $\hat k$ методом максимального правдоподобия. 
...
\begin{problem}
Величины $X_1$, $X_2$, \ldots, $X_n$ независимы и равномерно распределены на отрезке $[0;\theta]$, где $\theta>1$.
\begin{enumerate}
...
\item Как изменятся ответы на пункты (а) и (б), если исследователь не знает значений самих $X_i$, 
...
\begin{enumerate}
\item Постройте оценку максимального правдоподобия $\hat{p}_{ML}$.
\item Найдите ожидаемую и наблюдаемую информацию Фишера. Постройте график функции $I(p)$.
...
\begin{problem}
Величины $X_i$ независимы и одинаково распределены. Пусть $I_{X_i}(\theta)$ - информация Фишера о $\theta$, получаемая при наблюдении $X_i$.
\begin{enumerate}
\item  Верно ли, что $I_{X_1}(\theta)=I_{X_2}(\theta)$?
\item  Как найти $I_{X_1, \ldots ,X_n}(\theta)$ зная $I_{X_i}(\theta)$?
\end{enumerate}
...
\begin{problem}
  Величины $X_1$, \ldots , $X_n$ независимы и одинаково распределены с функцией плотности $f(t)=\frac{\theta \left(\ln  t\right)^{\theta -1}}{t} $  при  $t\in
\left[1;e\right]$. 
По выборке из 100 наблюдений оказалось, что $\sum{\ln(\ln(X_i))}=-30$
\begin{enumerate}
...
\begin{problem}
  Величины $X_1$, \ldots , $X_n$ независимы и одинаково распределены с функцией плотности $\frac{\theta e^{-\frac{\theta^2}{2t}}}{\sqrt{2\pi t^{3}
}}$ при $t\in \left[0;+\infty \right)$. По выборке из 100 наблюдений оказалось, что $\sum{1/X_i}=12$
\begin{enumerate}
...
\begin{problem}
  Известно, что $X_i$ независимы, $\E(X_i)=5$, $\Var(X_i)=4$ и $n$ велико. 
  Как примерно распределены следующие величины:
\begin{enumerate}
\item $\bar X_n$,
\item $Y_n=(\bar X_n+3)/(\bar X_n+6)$,
\item $Z_n=\bar X_n^2$,
\item $W_n=1/\bar X_n$
\end{enumerate}
...
\item По 100 наблюдениям оказалось, что $\sum X_i=25$. 
Посчитайте численное значение $\hat{a}$ и оцените дисперсию случайной величины $\hat{a}$.
\end{enumerate}
...
\item Выразите оценку дисперсии оценки $\hat{p}$ через достаточную статистику $T$.
...
\begin{problem}
Величины  $X_1$  и  $X_2$  независимы и равномерны на
...
При каких $k_1$  и  $k_2$  оценка  $\hat a=k_1 + k_2 X$ неизвестного параметра $a$  будет несмещённой?
\begin{sol}
...
\begin{problem}
  ...
  \begin{enumerate}
    ...
    \item Постройте 95\%-ый доверительный интервал для $a$ вида $[0;kX]$.
  \end{enumerate}  
\begin{sol}
...
\begin{enumerate}
  ...
  \item Постройте 90\%-ый доверительный интервал для $\mu$ вида $[0;kX]$.
\end{enumerate}
\begin{sol}
\end{sol}
\end{problem}
...
\begin{problem}
  ...
  \begin{enumerate}
    ...
    \item Постройте 90\%-ый доверительный интервал для $\mu$ вида $[\bar X_{10}-k;\bar X_{10}+k]$.
  \end{enumerate}  
\begin{sol}
\end{sol}
\end{problem}
