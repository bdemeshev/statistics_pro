...
\toprule
 $x$ & $0$ & $1$ & $2$ \\
\midrule
 $\P(X = x)$ & $1.3 - 3\theta$ & $2\theta-0.2$ & $\theta - 0.1$ \\
\bottomrule
\end{tabular}
...
Рыбак поймал 100 рыб и среди пойманных 100 рыб он посчитал количества $N_{\text{карась}}$, $N_{\text{окунь}}$, $N_{\text{щука}}$, $N_{\text{налим}}$.
\begin{enumerate}
...
Фарш фасуется упаковками по $a$ грамм, поэтому нерасфасованный остаток в $i$-ый день, $X_i$, 
