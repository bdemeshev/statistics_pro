...
% TODO: вернуть обратно простые \mid 

% как обозначить p-value? P(S_{\text{new}} \geq S \mid S, H_0)? ! Да!
...
\begin{problem}
Величины $X_1$, $X_2$, \ldots, $X_n$ независимы и их функции плотности равны 
\[ 
f(x)=
\begin{cases}
...
\begin{problem}
В банке 10 независимых клиентских «окошек». 
В момент открытия в банк вошло 10 человек. 
Каждый клиент встал к отдельному окошку. 
Других клиентов банке в этот день не было. 
...
\begin{enumerate}
\item Менеджер записал время обслуживания клиента в каждом окошке. 
Окошко №1 обслужило своего клиента за 10 минут, окошко №2 обслужило своего клиента за 20 минут; 
оставшуюся часть записей менеджер благополучно затерял. 
\item Менеджер наблюдал за окошками в течение получаса и записывал время обслуживания клиента. 
Окошко №1 обслужило своего клиента за 10 минут, окошко №2 обслужило своего клиента за 20 минут; 
остальные окошки еще обслуживали своих первых клиентов в тот момент, когда менеджер удалился. 
\item  Менеджер наблюдал за окошками в течение получаса. 
За эти полчаса два окошка успели обслужить своих клиентов. 
Остальные окошки еще обслуживали своих первых клиентов в тот момент, когда менеджер удалился. 
\item Менеджер наблюдал за окошками и решил записать время обслуживания первых двух клиентов. 
Через  10 минут от начала работы был обслужен первый клиент в одном из окошек, 
через 20 минут от начала работы был обслужен второй клиент. 
Сразу после того, как был обслужен второй клиент менеджер прекратил наблюдение. 
\item Одновременно с открытием банка началась деловая встреча директора банка с инспектором по охране труда. 
Время проведения таких встреч — случайная величина, имеющая экспоненциальное распределение со средним временем 30 минут. 
За время проведения встречи было обслужено два клиента. 
\item Одновременно с открытием банка началась деловая встреча директора банка с инспектором по охране труда. 
Время проведения таких встреч — случайная величина, имеющая экспоненциальное распределение со средним временем 30 минут. 
За время проведения встречи было обслужено два клиента, один за 10 минут, второй — за 20 минут. 
\item Изменим условие: в банке 11 окошек, при открытии банка вошло 11 клиентов. Других клиентов в этот день не было. 
Клиент попавший в окошко №11 смотрел за остальными. 
Раньше клиента из окошка  №11 освободилось двое клиентов: за 10 минут и за 20 минут. 
\end{enumerate} 
\begin{sol}
\end{sol}
\end{problem}
...
Имеется выборка из 1000 наблюдений по жителям столицы. 
Если возможно, постройте 90\% доверительный интервал для $\lambda$ в следующих случаях.
\begin{enumerate}
...
\item В столице селятся только люди с доходом больше $m$ тыс. рублей, где $m$ - неизвестная константа. 
При этом постройте также и 90\% доверительный интервал для $m$.
\item В столице живут 10\% самых богатых жителей страны.
\item В столице живут $p$\% самых богатых жителей страны, где $p$ - неизвестная константа. 
Постройте также 90\% доверительный интервал для $p$.
\end{enumerate}
\begin{sol}
\end{sol}
\end{problem}
...
\begin{problem}
Исследователь Михаил предполагает, что все виды медведепришельцев встречаются
...
Приведём для полноты ошибочное решение:

Количество медведей велико и все они встречаются равновероятно, то $p_{1}=p_{2}=p_{3}=1/n$. 
Из выборки известно, что число видов космомедведей не меньше трёх. Потому $\hat{n} \ge 3$.

$L(n)=\left(\frac{1}{n}\right)^{2} \cdot \frac{1}{n}\cdot\frac{1}{n}=n^{-4}$

Эта функция убывает по $n$, отсюда точка максимума равна $\hat{n}=3$.
...
\begin{problem}
Величины $X_i$ независимы и имеют закон распределения:
...
\begin{enumerate}
  \item Постройте любую состоятельную оценку для $a$. 
  \item Если возможно, постройте несмещённую оценку для $a$.
\end{enumerate}
...
Рассмотрим несколько оценок для параметра $a$. 
  \[
  \hat a_1 = 2 \bar X, \quad \hat a_2 = \frac{2(n+1)}{n} \bar X, \quad \hat a_3 = R, \quad \hat a_4 = \frac{n+1}{n} R.
  \]
  \begin{enumerate}[resume]
...
\begin{problem}
  Криминальный город Nsk полностью поделён кланами. 
  Мирных жителей не осталось совсем, каждый житель принадлежит одному из кланов. 
  Число кланов $k$ неизвестно. Размер каждого клана $y_i$ неизвестен. 

  Глеб Жеглов и Володя Шарапов могут отловить $n$ случайных жителей (возможно, с повторами) 
  и узнать у них размеры их кланов, $w_1$, $w_2$, \dots, $w_n$. 
  
  Как Глебу и Володе построить состоятельную оценку $\bar y$?
  \begin{sol}
...
\begin{problem}
Величины $X_1$, $X_2$, \dots, $X_n$ независимы и биномиально распределены $\dBin(a, p)$ с известным числом испытаний $a$ и неизвестной вероятностью успеха $p$. 

Рассмотрим оценку $\hat b_n = \bar X (a - \bar X)/a$ для неизвестной дисперсии $b = \Var(X_i)$.

\begin{enumerate}
  \item Выразите параметр $b$ через неизвестный параметр $p$ и известный параметр $a$. 
  \item Является ли оценка $\hat b_n$ несмещённой? Найдите величину смещения, если оценка является смещённой. 
  \item Является ли оценка $\hat b_n$ состоятельной?
\end{enumerate}
\begin{sol}
  \begin{enumerate}
    \item $b = a p(1-p)$;
    \item оценка смещённая;
    \item оценка состоятельная.
  \end{enumerate}  
  \end{sol}
\end{problem}
...
\begin{problem}
Величины $X_1$, $X_2$, \dots, $X_n$ независимы и нормально распределены $\cN(7; \sigma^2)$ с неизвестной дисперсией $\sigma^2$.

Рассмотрим оценку $\hat\sigma = c_1 \sum_{i=1}^n \abs{X_i - c_2}/n$ для стандартной ошибки $\sigma$.

\begin{enumerate}
  \item При каких значениях $c_1$ и $c_2$ оценка $\hat\sigma$ является несмещённой?
  \item При каких значениях $c_1$ и $c_2$ оценка $\hat\sigma$ является состоятельной?
\end{enumerate}
  \begin{sol}
  \end{sol}
\end{problem}
...
\begin{problem}
Возможны ли следующие ситуаций?
\begin{enumerate}
  \item Оценка $\hat\theta_n$ является несмёщенной при нечётных $n$ и смещённой при чётных $n$, а вся последовательность оценок является состоятельной;
  \item Оценка $\hat\theta_n$ является несостоятельной при нечётных $n$ и состоятельной при чётных $n$, а вся последовательность оценок является несмещённой.
\end{enumerate}
  \begin{sol}
    \begin{enumerate}
      \item да;
...
\begin{problem}
Величины $Y_1$, $Y_2$, $Y_3$ независимы и нормально распределены $\cN(0;\sigma^2)$ с неизвестной дисперсией $\sigma^2$.

Значение функции плотности нормального распределения $\cN(0;\sigma^2)$ в известной точке $x$ является неизвестным параметром $f(x)$.

Рассмотрим оценку $\hat f(x)$:
\[
\hat f(x) = \begin{cases}
c/\sqrt{Y_1^2 + Y_2^2 + Y_3^2}, \text{ если } x^2 \leq Y_1^2 + Y_2^2 + Y_3^2; \\
0, \text{ иначе.}
\end{cases}
\]

\begin{enumerate}
  \item Запишите $f(x)$ через известный параметр $x$ и неизвестный параметр $\sigma^2$.
  \item Подберите константу $c$ так, чтобы оценка $\hat f(x)$ была несмещённой для $f(x)$. 
\end{enumerate}
...
\[
  \E(S_1) = 0, \quad \Var(S_1) = \E(S_1^2) = - \E\left(\pdv{S_1}{\theta}\right), \quad \Cov(S_1, \hat \theta) = \pdv{\E(S_1)}{\theta}.  
\]  
Информация Фишера о неизвестном параметре $\theta$, содержащаяся в одном наблюдении:
\[
I_1(\theta) = \Var\left(\pdv{\ln f(X_1)}{\theta}\right) = \E\left(\left(\pdv{\ln f(X_1)}{\theta}\right)^2\right)= - \E\left(\pdv[2]{\ln f(X_1)}{\theta}\right)
\]
Для независимых наблюдений $I_n(\theta) = nI_1(\theta)$.

Неравенство Рао~— Крамера («слишком хорошей оценки не бывает»)
\[
\Var(\hat \theta) \geq \frac{\left(\Cov\left(\pdv{\ln f(X_1, \ldots, X_n)}{\theta}, \hat\theta\right)\right)^2}{I_n(\theta)} = \frac{\left(\pdv{\E(\hat\theta)}{\theta}\right)^2}{I_n(\theta)}
\]
\end{leftbar}
...
\begin{enumerate}
  ...
  \item Найдите $\E\left(\pdv{S_1}{\theta}\right)$.
...
\begin{problem}
  ...
  \begin{enumerate}
    ...
    \item Найдите $\E\left(\pdv{S_1}{a}\right)$.
...
\begin{sol}  
  Удаляем из колоды валетов, дам, королей и тузов.
...
\begin{problem}
  ...
  \begin{enumerate}
    \item Какова вероятность того, что интервал $[\bar X_{10} - 0.7;\bar X_{10} + 0.7]$ накроет неизвестное $\mu$? 
    \item Постройте 90\%-й доверительный интервал для $\mu$ вида $[\bar X_{10}-k;\bar X_{10}+k]$.
  \end{enumerate}  
\begin{sol}
$k \approx 1.65 / sqrt{10}$
\end{sol}
\end{problem}
...
\begin{enumerate}
  ...
  Величины $\mu$ и $\sigma$ неизвестны, $\bar X = 60$, а несмещённая оценка дисперсии равна $16$.
  \item Постройте 95\%-й доверительный интервал для $\mu$.  
  Величина $\mu$ неизвестна, $\bar X = 60$ и $\sigma = 5$.
  \item Постройте 90\%-й доверительный интервал для $\sigma$.  
  Величина $\sigma$ неизвестна, $\mu = 70$ и $\sum (X_i - 70)^2 = 800$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Известно, что оценка $\hat\mu$ оказалась очень-очень далеко от $\mu$. 
  У кого больше условная вероятность накрыть интервалом истинное значение $\mu$?
  \item Известно, что оценка $\hat \mu$ оказалась очень-очень близко к $\mu$. 
  У кого больше условная вероятность накрыть интервалом истинное значение $\mu$?  
...
\begin{problem}
Величины $Y_1$, \ldots, $Y_n$ независимы и имеют распределение Бернулли с неизвестным $p$,
$\hat p = \bar Y$. 
...
\begin{sol}
\begin{enumerate}
  \item Интервал Вальда 
  \[
  \hat p \pm \zcr \sqrt{\frac{\hat p (1 - \hat p)}{n}}  
  \]
  \item Интервал Вильсона можно записать по-разному, например, так
  \[
    \hat p_w \pm \zcr \cdot \sqrt{\frac{u \hat p (1 - \hat p) + (1 - u) (1/2)^2 }{n_w}},
  \]
  где $\hat p_w = u \hat p + (1 - u) (1/2)$, $u = \frac{n}{n + \zcr^2}$, $(1-u) = \frac{\zcr^2}{n + \zcr^2}$, 
  $n_w = n + \zcr^2$.
  \item 
  \[
  \hat p_w = u \hat p + (1 - u) (1/2), 
  \]
  где $u = \frac{n}{n + \zcr^2}$, $(1-u) = \frac{\zcr^2}{n + \zcr^2}$.
  \item 
  \[
    \hat p_w = \frac{\sum Y_i + \zcr^2/2}{n + \zcr^2},
  \]
  то есть мы добавляем в выборку $f = \zcr^2/2$ вымышленных $1$ и столько же вымышленных $0$. 
  \item 
  \[
    \hat p_w \pm \zcr \cdot \sqrt{\frac{u \hat p (1 - \hat p) + (1 - u) (1/2)^2 }{n_w}},
  \]
  ...
  $n_w = n + \zcr^2$.
  \item 
  При уровне доверия 95\% критическое $\zcr \approx 1.96 \approx 2$ и центры интервалов Вильсона и Агрести~— Коулла практически совпадают. 
  \item В силу неравенства Йенсена интервал Агрести~— Коулла немножко шире. 
  \item Интервалы Вильсона и Агрести~— Коулла по сравнению с интервалом Вальда 
  должны давать фактическую вероятность накрытия ближе к номинальной при $p$ близких к $0$ или $1$.
\end{enumerate}
...
\begin{problem}
  Сёгун Минамото-но Ёритомо хочет отбирать на службу только опытных самураев. 
    ...
    \begin{enumerate}
        \item Найдите вероятности ошибок первого и второго рода при $a = 3.5$.
        \item Постройте кривую зависимости ошибки второго рода от ошибки первого рода при различных порогах $a$.
        \item Как изменятся ответы на пункты (а) и (б), если Ёримото даёт каждому испытуемому две попытки и сравнивает с порогом $a$ наиболее удачную попытку?
    \end{enumerate}
Источник: экзамен декабрь 2025, вшэ, пми. 
\begin{sol}
\end{sol}    
\end{problem}
...
\begin{problem} 
  Перед нами результаты Васиных домашних заданий, $y$ (в баллах от $0$ до $100$), 
  и информация о том, был ли ретроградным меркурий по время выполнения домашнего задания, $x$ («yes» или «no»).
\begin{minipage}{\textwidth}
\begin{tabular}{clllll}
\toprule
Оценка $y$ & $60$ & $70$ & $80$ & $40$ & $70$ \\
Ретроградный меркурий $x$ & yes & no & yes & no & no \\
\bottomrule
\end{tabular}
\end{minipage}
У Васи есть смутные подозрения, что ретроградный меркурий благоприятен хорошей оценке за домашнюю работу. 
    \begin{enumerate}
        \item Найдите разницу между средней оценкой с ретроградным меркурием и без него, $S = \bar y_{\tyes} - \bar y_{\tno}$.
    \end{enumerate}
Рассмотрим скучную гипотезу $H_0$: ретроградный меркурий не влияет на оценку, и Васину гипотезу $H_a$: ретроградный меркурий благоприятен хорошей оценке. 
Если верить в $H_0$, то разумно предположить, что переменная $x$ — это просто случайная равновероятная перестановка двух «yes» и трёх «no».
\begin{enumerate}[resume]
  \item Найдите $P$-значение, то есть вероятность получить в новом эксперименте значение статистики $S_{\tnew}$ такое же как $S$ или выше, предполагая, что в новом эксперименте верна гипотеза $H_0$,
  \[
  \text{P-value} = \P(S_{\tnew} \geq S \mid S, H_0).
  \]
  \item Вася решил отвергнуть гипотезу $H_0$, если $P$-значение окажется меньше $0.05$, и не отвергнуть её в противном случае. 
  К какому статистическому выводу придёт Василий?
\end{enumerate}
\begin{sol}
\end{sol}    
\end{problem}
...
\begin{problem}
  ...
  Заранее были заготовлены 100 пузырьков. 
  Пузырьки занумеровали и расставили в случайном порядке, затем в 50 из них налили дистилированную
  ...
  Далее в таверне Красный Петух собрались 120 уважаемых жителей города, 
...
\begin{problem}
Вовочка проверяет гипотезу $H_0$: $\mu = 0$ против альтернативной $\mu > 0$ с помощью очень простого критерия. 
Если $\bar X > 0$, то Вовочка отвергает $H_0$, а иначе — не отвергает $H_0$.
У Вовочки очень-очень большое количество наблюдений, наблюдения $X_i$ независимы и одинаково распределены.
\begin{enumerate}
  \item Чему примерно равна вероятность ошибки первого рода?
  \item Чему примерно равна вероятность второго рода?
  \item Чему равна вероятность $\P(\plim_{n\to\infty} \bar X > 0)$ при верной $H_0$?
  \item Чему равна вероятность $\P(\plim_{n\to\infty} \bar X > 0)$ при верной $H_a$?
\end{enumerate}
  \begin{sol}
    \begin{enumerate}
      \item $1/2$
...
\begin{problem}
Экзамен по математической статистике писали $50$ студентов. 
Во время экзамена преподаватель выпустил из аудитории на пять минут одного студента из $50$. 
Именно этот студент получил максимальный балл за экзамен. 

На уровне значимости $5\%$ проверьте гипотезу $H_0$ о том, что совпадение случайно, против альтернативной гипотезы о загадочной связи между выходом из аудитории и высоким баллом.
Укажите точное $P$-значение. 
  \begin{sol}
    $P$-значение равно $1/50=0.02$, гипотеза $H_0$ отвергается.
  \end{sol}
\end{problem}
