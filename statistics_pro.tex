...
Это задачки по статистике к семинарам исследовательского потока факультета экономики вшэ. 
...
подборку прошлых экзаменов — \url{https://github.com/bdemeshev/probability_hse_exams},
...
\begin{problem}
 Исследовательница Мишель подбрасывает кубик\index{кубик} $100$ раз. Пусть $X_1$ — количество выпадений единицы, а $X_6$ — количество выпадений шестёрки.
\begin{enumerate}
...
Ошибочно утверждать, что $S \sim Bin\left(100, p=\frac{100}{n}\right)$, так как вероятность 
...
\end{problem}
\begin{problem}
  Пусть $X_{i}$ независимы и одинаково распределены $\cN(\alpha,2\alpha)$ \\
...
\begin{problem}
  Пусть $X_i$ — независимы и нормальны $\cN(\mu,1)$. Какова вероятность того, что интервал $[\bar{X}_{10}-1;\bar{X}_{10}+1]$ накроет неизвестное $\mu$? Постройте 90\%-ый доверительный интервал для $\mu$ вида $[\bar{X}_{10}-k;\bar{X}_{10}+k]$.
\begin{sol}
\end{sol}
\end{problem}
...
\begin{sol}
  ...
  \todo[inline]{Можно превратить в задачку на данных с пунктами сделать так и так и выбрать, как верно.}
\end{sol}
\end{problem}
...
\begin{problem}
Величины $X_1$ и $X_2$ независимы и равномерны на отрезке $[0;a]$. Есть две гипотезы, $H_0$: $a=1$ и $H_a$: $a=2$. 
...
\begin{problem}
Бабушка Аксинья\index{герои!бабушка Аксинья} утверждает, что обладает сверхспособностями и умеет слышать Внутренний Голос. Стоит лишь Аксинье глянуть на стакан с водой, как Внутренний Голос подсказывает, что налито в стакан, Бон-Аква или Аква Минерале.
...
Борис решил на следующий день повторить эксперимент и снова посчитать $S_{new}$, 
количество клиентов из первых ста, которые выберут капуччино. 
\begin{enumerate}[resume]
  \item Найдите $\P(S_{new} \geq S \mid S=40, H_0)$ и $\P(S_{new} \geq S \mid S=40, H_a)$.
\end{enumerate}
...
\begin{problem}
Пусть ${X_1}, \ldots, {X_n}$ — случайная выборка из нормального распределения с неизвестным математическим ожиданием $\mu$ 
